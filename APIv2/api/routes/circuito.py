...
def create_circuito(circuito: CircuitoCreate, db: Session = Depends(get_db)):
    return circuito_repo.create(db, circuito)

@router.put("/{circuito_id}", response_model=CircuitoOut)
def update_circuito(circuito_id: int, circuito: CircuitoUpdate, db: Session = Depends(get_db)):
...
def delete_circuito(circuito_id: int, db: Session = Depends(get_db)):
