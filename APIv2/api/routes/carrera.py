...
def create_carrera(carrera: CarreraCreate, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    require_admin(current_user)
    return carrera_repo.create(db, carrera)

@router.put("/{carrera_id}", response_model=CarreraOut)
def update_carrera(carrera_id: int, carrera: CarreraUpdate, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    require_admin(current_user)
...
def delete_carrera(carrera_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    require_admin(current_user)
