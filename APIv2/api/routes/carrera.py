...
def create_carrera(carrera: CarreraCreate, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
...
def update_carrera(carrera_id: int, carrera: CarreraUpdate, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    current_user = require_admin(get_current_user(token=token, db=db))
...
def delete_carrera(carrera_id: int, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    current_user = require_admin(get_current_user(token=token, db=db))
