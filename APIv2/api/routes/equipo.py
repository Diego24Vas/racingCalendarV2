...
def create_equipo(equipo: EquipoCreate, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
...
def update_equipo(equipo_id: int, equipo: EquipoUpdate, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    current_user = require_admin(get_current_user(token=token, db=db))
...
def delete_equipo(equipo_id: int, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    current_user = require_admin(get_current_user(token=token, db=db))
